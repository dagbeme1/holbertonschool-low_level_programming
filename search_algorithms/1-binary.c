...
int binary_search(int *array, size_t size, int value)
{
	...
	/* Declare integer variables for counting, start, end, and mid */

	if (!array)
		return (-1); /* Return -1 if array is NULL */

	while (end >= start)
		/* Run the binary search loop while end is greater than or equal to start */
	{
		printf("Searching in array: ");
		/* Print a message indicating searching in array */

		for (i = start; i <= end; i++)
			/* Loop to print the elements in the current range */
		{
			printf("%i", array[i]);
			/* Print the current array element */

			if (i != end)
				/* Check if the current element is not the last one in the range */
				printf(", ");
		...
		if (array[mid] == value)
			...
			return (mid);
			/* Return the middle index if the value is found */

		if (value < array[mid])
			/* Check if the value is less than the middle element */
			end = mid - 1;
